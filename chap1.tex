...
Finding a practically shortest route on a large road network 
in a metropolis can be a challenge, especially when the road traffic
is known to be \emph{time-dependent}. A typical Bellman-Ford\cite{CLRS09} or
Dijkstra's algorithm\cite{Dij59} based on the assumption that the cost of each
route is constant do not work on time-dependent road networks.
